...
for (sg in 1:length(unif)) {
  ...
  load(paste0("./Results/Results_England_",unif[sg],".Rdata"))
  
  
  for (pr in 1:length(priors)) {
    eval(parse(text = paste0("res <- results$",priors[pr],".res")))
    
    tab.results[pr, 1] <- round(mean(res$summary$all.chains[paste0("MSS.r[", 1:S.area, "]"), "Mean"]*10^10),3)
    tab.results[pr, 2] <- round(mean(res$summary$all.chains[paste0("RMSS.r[", 1:S.area, "]"), "Mean"]*10^5),3)
    
    tab.results[pr, 3] <- round(max(res$summary$all.chains[paste0("MSS.r[", 1:S.area, "]"), "Mean"]*10^10),3)
    tab.results[pr, 4] <- round(max(res$summary$all.chains[paste0("RMSS.r[", 1:S.area, "]"), "Mean"]*10^5),3)
    
    tab.results[pr, 8] <- round(sum(res$summary$all.chains[paste0("MSS.r[", 1:S.area, "]"), "Mean"]*10^10)/sum((mean(data$crude.rate) - data$crude.rate)^2),3)
    ...
    if(priors[pr]%in%c( "iCAR", "pCAR")){
      ...
      inv.condvar <- MASS::ginv(tab.results[pr, 7]*inv.R + (1-tab.results[pr, 7])*diag(1, length(carto.nb)))
      tab.results[pr, 5] <- round(sum(tab.results[pr, 6]/diag(inv.condvar)),3)
    }
  ...
  latex_table <- xtable::xtable(tab.results, caption=paste("Results Lung cancer "), digits=3)
...
for (sg in 1:length(unif)) {
  data.results <- cbind(rep("crude rate", S.area), district, data$crude.rate)
  ...
  for (pr in 1:length(priors)) {
    ...
    data.results <- rbind(data.results,
                          cbind(rep(priors[pr], S.area), district, 
                                res$summary$all.chains[paste0("r[", 1:S.area, "]"), "Mean"]*10^5))

  }

  data.results <- data.frame(data.results)
  colnames(data.results) <- c("prior", "Code", "est.rate")
  ...
  data2 <- merge(carto, data.results, by = c("Code"))


  palet <- c("#ffffd4", "#fee391", "#fec44f", "#fe9929", "#d95f0e", "#993404")

  if(sg==2 | sg==4){
    usa_1 <- ggplot(data = data2, aes(group = Code)) +
...
ggplot2::ggsave(filename = paste0("./Maps_rates_England_1.pdf"),
                plot = ga1,
                device = "pdf",
                dpi = 600,
                width = 20,
...
ggplot2::ggsave(filename = paste0("./Maps_rates_England_2.pdf"),
                plot = ga2,
                device = "pdf",
                dpi = 600,
                width = 20,
